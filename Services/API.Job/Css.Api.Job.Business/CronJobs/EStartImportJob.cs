...
{
    /// <summary>
    /// The cron job for EStart import
    /// </summary>
    public class EStartImportJob : CronJobService
    {
        ...
        /// The logger
        /// </summary>
        private readonly ILogger<EStartImportJob> _logger;
        ...
        public EStartImportJob(IScheduleConfig<EStartImportJob> config, ILogger<EStartImportJob> logger, IHttpService httpService, IOptions<Jobs> jobs)
            : base(config.CronExpression, config.TimeZoneInfo)
        {
            _logger = logger;
            _httpService = httpService;
            _job = jobs.Value.Cron.First(x => x.Key.Equals("EStartImport"));
        }
        #endregion
        ...
        {
            _logger.LogInformation("EStartImportJob starts.");
        ...
        public override async Task Process(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogInformation($"EStartImport run at {startTime} completed. Response details:\n{response}");
                }
                else
                {
                    _logger.LogError($"EStartImport run at {startTime} failed. Response details:\n{response}");
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"An exception occured in the EStartImport run at {startTime}. Expection details:\n{ex.Message}\n{ex.StackTrace}");
            }
        }
        ...
        {
            _logger.LogInformation("EStartImport is stopping.");
